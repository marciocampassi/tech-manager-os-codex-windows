...
- [ ] Task 1: Create `src/utils/email-guard.ts` (AC: 1)
  - [ ] Import `findSimilarEmail` from `./email-similarity.js` and `printWarning` from `./display.js`
  - [ ] Export `resolveEmailWithSimilarCheck(email, workspaceRoot): Promise<string>` — see exact implementation in Dev Notes
  - [ ] Do NOT re-implement levenshtein or scan logic — delegate to `findSimilarEmail`
- [ ] Task 2: Update `src/commands/member.command.ts` (AC: 4, 5, 6)
  - [ ] Remove `import { findSimilarEmail } from '../utils/email-similarity.js'`
  - [ ] Add `import { resolveEmailWithSimilarCheck } from '../utils/email-guard.js'`
  - [ ] Delete the `warnIfSimilarEmail` function (lines 12–28)
  - [ ] Member-creation path (line ~57): change `const email` → `let email`, then replace the `warnIfSimilarEmail` guard with `email = await resolveEmailWithSimilarCheck(email, ws);` (no `if (!shouldContinue) return`)
  - [ ] Type-first path (line ~200): `email` is already `let` — replace the `warnIfSimilarEmail` guard with `email = await resolveEmailWithSimilarCheck(email, ws);`
- [ ] Task 3: Update `src/commands/leadership.command.ts` (AC: 4, 5, 6)
  - [ ] Remove `import { findSimilarEmail }` → add `import { resolveEmailWithSimilarCheck }` from `email-guard.js`
  - [ ] Delete the `warnIfSimilarEmail` function (lines 12–26)
  - [ ] Line ~65 (`runLeadershipAdd`): `email = await resolveEmailWithSimilarCheck(email, ws);`
  - [ ] Line ~145 (`runLeadership1on1`): `email = await resolveEmailWithSimilarCheck(email, ws);`
  - [ ] Verify `email` is `let` at both call sites (both already are — resolved from prompt or arg)
- [ ] Task 4: Update `src/commands/team.command.ts` (AC: 4, 5, 6)
  - [ ] Remove `import { findSimilarEmail }` → add `import { resolveEmailWithSimilarCheck }` from `email-guard.js`
  - [ ] Delete the `warnIfSimilarEmail` function (lines 13–27)
  - [ ] Line ~123 (`runAdd`): `email = await resolveEmailWithSimilarCheck(email, ws);` — `email` is already `let`
- [ ] Task 5: Update `src/commands/project.command.ts` (AC: 4, 5, 6, 7)
  - [ ] Remove `import { findSimilarEmail }` → add `import { resolveEmailWithSimilarCheck }` from `email-guard.js`
  - [ ] Delete the `warnIfSimilarEmail` function (lines 11–25)
  - [ ] Line ~111 (`runProjectLinkMember`): `email` is `const` — change to `let email`, then `email = await resolveEmailWithSimilarCheck(email, ws);`
  - [ ] `runProjectLinkMembers` batch loop (lines ~155–157): change loop variable to `rawEmail` and push resolved email → `filteredEmails.push(await resolveEmailWithSimilarCheck(rawEmail, ws));`
  - [ ] Line ~203 (`runProjectLinkStakeholder`): `email` is `const` — change to `let email`, then `email = await resolveEmailWithSimilarCheck(email, ws);`
  - [ ] `runProjectLinkStakeholders` batch loop (lines ~247–249): same pattern as link-members
- [ ] Task 6: Update tests (AC: 8)
  - [ ] `tests/commands/member.command.test.ts`: change `jest.unstable_mockModule('../../src/utils/email-similarity.js', ...)` → mock `'../../src/utils/email-guard.js'` with `{ resolveEmailWithSimilarCheck: jest.fn().mockResolvedValue }` — see Dev Notes for the exact mock shape
  - [ ] `tests/commands/leadership.command.test.ts`: same change
  - [ ] `tests/commands/team.command.test.ts`: add `jest.unstable_mockModule('../../src/utils/email-guard.js', ...)` if needed (currently relies on real `findSimilarEmail` returning null for fake workspace — the real `resolveEmailWithSimilarCheck` will also return the email unchanged for a fake workspace, so may not need a mock)
  - [ ] `tests/commands/project.command.test.ts`: same as team
  - [ ] Verify all tests pass: `npm run test`
- [ ] Task 7: Validate (AC: 9)
