...
## Deferred from: code review of 9-37-init-installs-default-skills-bundle-packaging-and-registry-fallback (2026-06-20)

- Idle-only request timeout (no wall-clock deadline) in `fetchUrl` can hang init on a slow-trickle server. Pre-existing, shared with `tmr install`; Story 9.37 newly routes init through this path on the registry fallback. Consider an absolute deadline / `AbortController`. [`src/services/skill-registry.service.ts:34-36`]
- Serial fallback fetches can block init up to ~30s when all three bundled files are missing and the network is slow (no aggregate cap, no parallelization). Degraded-path only — requires a packaging regression to reach. [`src/services/init.service.ts` `installDefaultSkill`]
- Spinner reports "Default skills installed" even when 0 skills were installed; `installDefaultSkill` returns `void` and swallows all failures. By-design non-fatal contract (Story 2.4) and an explicit regression guard — revisit only if surfacing an install count is desired. [`src/commands/init.command.ts:249-255`]
- Registry 200 with a non-skill body (captive portal / GitHub error HTML) is installed verbatim as `SKILL.md` with version `0.0.0`. No content-shape/frontmatter validation. Pre-existing `fetchSkillContent`/`installSkill` behavior. [`src/services/skill-registry.service.ts:122-124`]
- `fetchUrl` does not follow 3xx redirects → the registry fallback silently skips a skill on any redirect. Pre-existing. [`src/services/skill-registry.service.ts:100-120`]
- Missing/garbled `<!-- version: -->` comment → `parseBundledVersion`/`parseVersion` default to `0.0.0` and `installSkill` overwrites the manifest unconditionally (no newer-version comparison on re-run/upgrade). Pre-existing. [`src/services/init.service.ts:77-80` + `installSkill`]
- `_readBundledSkill` path resolution (`new URL('..', import.meta.url)`) assumes a flat `dist/` layout one level under the package root; under `ts-node` dev or a non-flat build it fails the bundled read and silently falls back to the network. Production (published flat `dist/`) resolves correctly — verified via `npm pack`. Dev-only; pre-existing path logic (intentionally unchanged by 9.37). [`src/services/init.service.ts:316-324`]
